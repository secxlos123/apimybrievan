...
class ViewAuditrailSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        \DB::unprepared("CREATE VIEW auditrail_type_one AS
         select a.id
         	   , a.created_at
         	   , case when event = 'created' and lower(auditable_type) = 'app\\models\\eform' and slug = 'customer' then 'Tambah Eform via Nasabah' 
		     ...
		     when event = 'created' and lower(auditable_type) = 'app\models\appointment' and slug = 'customer' then 'Penjadwalan baru via Nasabah'
		     when event = 'created' and lower(auditable_type) = 'app\models\appointment' and slug = 'developer-sales' then 'Penjadwalan baru via AgenDev'
		     when event = 'created' and lower(auditable_type) = 'app\models\appointment' and role = 'ao' then 'Penjadwalan baru via AO'
