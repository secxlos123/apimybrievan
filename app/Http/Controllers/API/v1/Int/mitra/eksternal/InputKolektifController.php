...
namespace App\Http\Controllers\API\v1\Int\mitra\eksternal;
...
class InputKolektifController extends Controller
{
    ...
    public function store( GimmickRequest $request )
    {
		...
		$penilaian_update = DB::table('mitra_header')
            ->where('id_header', $data_header['input_kolektif']['id_header'])
            ->update(['jumlah_pekerja' => $data_header['input_kolektif']['jumlah_pekerja']]);
