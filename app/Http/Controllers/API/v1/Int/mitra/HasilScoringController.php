...
class HasilScoringController extends Controller
{
    ...
    public function store( GimmickRequest $request )
    {
		...
		$penilaian_update = DB::table('scoring_mitra')
            ->where('id_scoring', $data_header['hasil_scoring']['id_header'])
            ->update(['rekomendasi_unit_kerja' => $data_header['hasil_scoring']['rekomendasi_unit_kerja']]);
